import { PactV3Factory } from "@you54f/nestjs-pact"
import { Test } from "@nestjs/testing"
import { HttpStatus } from "@nestjs/common"
import { AppModule } from "../src/app.module"
import { PactModule } from "./pact/pact.module"
import { Matchers, MatchersV3, PactV3 } from "@pact-foundation/pact"
...
describe('Pact', () => {
  ...
  const { iso8601DateTimeWithMillis } = Matchers;
  ...
  const animalListExpectation = eachLike(animalBodyExpectation,  MIN_ANIMALS);
  ...
  describe('when a call to list all animals from the Animal Service is made', () => {
    describe('and the user is not authenticated', () => {
      beforeAll(() =>
        provider.addInteraction({
          states: [{description:'is not authenticated'}],
    ...
    describe('and the user is authenticated', () => {
      describe('and there are animals in the database', () => {
        beforeAll(() =>
          provider.addInteraction({
            states: [{description:'Has some animals'}],
            uponReceiving: 'a request for all animals',
            withRequest: {
  ...
  describe('when a call to the Animal Service is made to retreive a single animal by ID', () => {
    describe('and there is an animal in the DB with ID 1', () => {
      beforeAll(() =>
        provider.addInteraction({
          states: [{description:'Has an animal with ID 1'}],
    ...
    describe('and there no animals in the database', () => {
      beforeAll(() =>
        provider.addInteraction({
          states: [{description:'Has no animals'}],
