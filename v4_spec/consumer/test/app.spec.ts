    ...
    describe('and the user is not authenticated', () => {
      it('returns a 401 unauthorized', async () => {
        await provider
          ...
          .withRequest('GET', '/animals/available')
          .willRespondWith(HttpStatus.UNAUTHORIZED)
          .executeTest(async (mockServer) => {
            process.env.API_HOST = mockServer.url;
            await expect(animalsService.suggestion(suitor)).rejects.toThrow();
          });
      });
    });
    ...
    describe('and there no animals in the database', () => {
      it('returns a 404', async () => {
        await provider
          ...
          .withRequest('GET', '/animals/100', (builder) => {
            builder.headers({ Authorization: 'Bearer token' });
          })
          .willRespondWith(HttpStatus.NOT_FOUND)
          .executeTest(async (mockServer) => {
            process.env.API_HOST = mockServer.url;
